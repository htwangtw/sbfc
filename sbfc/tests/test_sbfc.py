...
def _make_data():
    ...
    group_design_matrix = pd.DataFrame(adhd_dataset.phenotypic)[
        ["Subject"]
    ]
    group_design_matrix = group_design_matrix.rename(columns={"Subject": "subject_label"})
...
def test_sbfc():
    func_img, group_design_matrix, group_confounds, group_contrast = _make_data()
    # Prepare seed
    pcc_coords = (0, -53, 26)
    first_m, first_con, s_m = seed_base_connectivity(
        func_img, pcc_coords, group_confounds, group_design_matrix, group_contrast
    )
